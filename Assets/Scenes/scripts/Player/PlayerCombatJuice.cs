...
public class PlayerCombatJuice : MonoBehaviour
{
    ...
    /// <summary>
    /// Spawns a glowing procedural slash arc in front of Lumi during an attack swing.
    /// </summary>
    public void SpawnSlashArc(Vector3 position, float facingDirection, bool isHeavy)
    {
        return; // Disabled to respect clean native attack animations
        GameObject arcObj = new GameObject(isHeavy ? "HeavySlashArc" : "LightSlashArc");
        arcObj.transform.position = position + new Vector3(facingDirection * 0.8f, 0.2f, 0f);

        // Flip arc horizontally if facing left
        Vector3 localScale = new Vector3(facingDirection * (isHeavy ? 1.6f : 1.2f), isHeavy ? 1.5f : 1.1f, 1f);
        arcObj.transform.localScale = localScale;

        SpriteRenderer sr = arcObj.AddComponent<SpriteRenderer>();
        sr.sprite = slashArcSprite;
        sr.color = isHeavy ? new Color(0.95f, 0.4f, 1.0f, 0.95f) : new Color(0.65f, 0.35f, 1.0f, 0.88f); // Violet / Arcane Magenta
        sr.sortingOrder = 15;

        // Animate slash arc expansion & fade
        StartCoroutine(AnimateSlashArc(arcObj, sr, isHeavy));
    }

    private IEnumerator AnimateSlashArc(GameObject arcObj, SpriteRenderer sr, bool isHeavy)
    {
        float duration = isHeavy ? 0.16f : 0.12f;
        float elapsed = 0f;
        Vector3 startScale = arcObj.transform.localScale;
        Vector3 targetScale = startScale * 1.35f;
